...
<body id="bodies" style="background-image: url('/images/ac_ito/escudoNac.png'); background-position: left; background-repeat: no-repeat; background-attachment: fixed; padding-bottom: 10px;">
	<div id="app">
		...
		<nav class="navbar navbar-expand-md shadow-sm" style="background-color: transparent;"> <!-- navbar-dark bg-info shadow-sm" -->
            ...
            <li class="nav-item">
							<a class="nav-link" href="{{ url('Est') }}"><i class="fa fa-fw fa-lg  fa-home"></i>Inicio</a>
						</li>
            <li class="nav-item dropdown">
							<a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"><i class="fa fa-university"></i>Actividades</a>
							<div class="dropdown-menu border-bottom-0 border-left-0 border-right-0" style="background-color: white;">
								<a class="dropdown-item text-primary" href="{{ url('Est/micarrera') }}"> Mi carrera </a>
								<a class="dropdown-item text-primary" href="{{ url('Est/accarreras') }}"> Act. por Carreras </a>
								@foreach($tipos as $t)
									<a class="dropdown-item text-primary" href="{{ url('Est/actip').$t->id_tipo }}"> Act. {{ $t -> nombre}} </a>
								@endforeach
								<a class="dropdown-item text-primary" href="{{ url('Est/lineamiento') }}"> Lineamiento TecNM </a>
							</div>
						</li>
            <li class="nav-item">
							<a class="nav-link" href="{{ url('Est/cursando') }}"><i class="fa fa-fw fa-lg  fa-television"></i>En curso</a>
						</li>
            <li class="nav-item">
							<a class="nav-link" href="{{ url('Est/historial') }}"><i class="fa fa-fw fa-archive"></i>Historial</a>
						</li>
						...
						<li class="nav-item dropdown">
							...
							<div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
								<a class="dropdown-item text-primary" href="{{ url('Est/perfil') }}"><i class="fa fa-fw fa-lg  fa-cog"></i>Mi perfil</a>
								
								<a class="dropdown-item text-primary" href="{{ url('logoutE') }}" onclick="event.preventDefault(); $('#logout-form').submit();">
								<strong> <i class="fa fa-fw fa-lg  fa-sign-out"></i> Cerrar sesión </strong>
								</a>

								<form id="logout-form" action="{{ url('logoutE') }}" method="POST" style="display: none;">
									@csrf
								</form>
							</div>
						</li>
		...
		<main class="py-4">
      		<center> <h5> Actividades Complementarias </h5> </center>
			  <br>
			@yield('content')
		</main>
